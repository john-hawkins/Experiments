...
for(d in seq(1,dataset)) {
    ...
    filescore <- paste("data/train_set_part", d+1, ".rds", sep='')    
    train           <- readRDS(filetrain)
    score           <- readRDS(filescore)
    replaceNAs( train, features, 0)
    replaceNAs( score, features, 0)
    just.train      <- train[,c( target, features)]
    just.score      <- score[,c( target, features)]
    train.rats      <- addLookupRatios(just.train)
    score.rats      <- addLookupRatios(just.score)
    train.diffs     <- addLookupDiffs(train.rats)
    score.diffs     <- addLookupDiffs(score.rats)
    ...
    # NOW SCORE THE SCORE SET
    score.h2o       <- as.h2o(score.diffs)    
